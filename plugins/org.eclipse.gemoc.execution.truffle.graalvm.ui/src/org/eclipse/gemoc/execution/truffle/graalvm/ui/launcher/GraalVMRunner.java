...
/**
 * Adapted from StandardVMRunner
 *
 */
public class GraalVMRunner extends AbstractVMRunner {
	 ...
	 */
	public GraalVMRunner(IVMInstall vmInstance, String mode) {
...
//			}
	...
	protected class CommandDetails {
		...
		public void setDAPPort(int port) {
			this.dapPort = port;
		}
	...
	private CommandDetails getCommandLine(VMRunnerConfiguration config, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		...
		if (ILaunchManager.RUN_MODE.equals(mode)) {
			...
			arguments.add("--dap.Suspend=true");
			//--dap.WaitAttached
		}
		...
		if (map != null) {
			...
			if (JavaRuntime.isModularJava(fVMInstance)) {
				if (prependBootCP != null && prependBootCP.length > 0) {
					prependBootCP = null;
					Activator.log(LaunchingMessages.RunnerBootpathPError);
				}
				if (bootCP != null && bootCP.length > 0) {
					bootCP = null;
					Activator.log(LaunchingMessages.RunnerBootpathError);
				}
			}
